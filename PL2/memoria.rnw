...
Al igual que en el Ejercicio 1.1, ambas variables se interpretan como coordenadas de un punto en el plano bidimensional \((\text{Teoría}, \text{Laboratorio})\). En este caso, las calificaciones no son enteras, sino valores reales, lo que permite representar con mayor precisión diferencias intermedias en el rendimiento de los estudiantes. Cada estudiante queda, por tanto, representado como un par ordenado en \(\mathbb{R}^2\), sobre el que se aplicará posteriormente un método de \textbf{clusterización jerárquica aglomerativa}. 
